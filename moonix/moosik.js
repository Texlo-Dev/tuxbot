...
const commands = {
	'play': (msg) => {
		if (queue[msg.guild.id] === undefined) return msg.channel.sendMessage(`:fire: Hmm..I couldn't find a song to play. Add one with ${tokens.prefix}add`);
		...
		(function play(song) {
			console.log(song);
			if (song === undefined) return msg.channel.sendMessage(':fire: Queue is empty').then(() => {
	...
	'add': (msg) => {
		...
		if (url == '' || url === undefined) return msg.channel.sendMessage(`:fire: Hmm...This link didn't work.. Add another one with ${tokens.prefix}add`);
	...
	'queue': (msg) => {
		if (queue[msg.guild.id] === undefined) return msg.channel.sendMessage(`:fire: Aw, no songs! Add one with ${tokens.prefix}add`);
		let tosend = [];
