...
const commands = {
	'play': (msg) => {
		if (queue[msg.guild.id] === undefined) return msg.channel.sendMessage(`Hmm..I couldn't find a song to play. Add one with ${tokens.prefix}add`);
	...
	'queue': (msg) => {
		if (queue[msg.guild.id] === undefined) return msg.channel.sendMessage(`Aw, no songs! Add one with ${tokens.prefix}add`);
		let tosend = [];
